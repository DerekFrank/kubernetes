# Provisioning Pipeline — exploration (NOT decided)

**Status:** scratch / thinking-out-loud. A *second* structural approach to the provisioning half, distinct from the current inline `solve()` follow-up. Captured to argue its merits, not to commit to it.

## Why a second approach at all

Bind-first (D17) changed what the provisioning problem *is*. Once binding to retained capacity is handled structurally and first, the provisioning step no longer has to reason about existing nodes at all — it answers one narrow question: **"given this set of pods that could not bind, what new capacity should we create?"** That is a pure function of `(pods, offerings)`. Two structural advantages fall out, and they are the whole reason to consider a pipeline:
...
The pipeline below is one way to structure that pure, parallel provisioning solve. Whether *this* set of phases is the right factoring is the open question.
...
An earlier draft had a `Validate(Solution) → bool` gate before Select. **Dropped.** Two kinds of validity, neither warrants a phase:

- **Internal validity** (does the solution satisfy pod + offering constraints?) is **time-invariant and fused into Solve** — a solver constructs only-feasible solutions, exactly as Karpenter's `Solve` does, exactly as a Filter's contract is "passes only feasible nodes." A solver that emits an infeasible solution is *buggy*, not *policy*. And solvers **nominate binding** — trust is already fully extended to their output; re-checking it downstream is incoherent. So internal validity is a **solver contract**, not a pipeline stage. (Skipping it also removes redundant work: the solver already enforced fit/topology/narrowing *during* construction.)
  - *Sharp edge to remember:* the candidate set is OR-ed and Select is an argmin on cost, and violating a constraint is usually **cheaper** (tighter packing). So a constraint-violating candidate doesn't slip through — it **wins**. Dropping Validate therefore places *higher-stakes* trust in solvers than kube-scheduler places in Filters (whose violations merely *add* a bad option; ours *dominate*). Fine for trusted/default solvers; an **opt-in** feasibility gate is the escape hatch for untrusted third-party solvers — present by wiring, absent by default (like Select being identity at N=1).
- **External validity** (staleness — did the world move while we batched?) **cannot be a pipeline phase**: any mid-pipeline check is itself stale by commit time (TOCTOU). Staleness is a **commit-boundary** concern, reconciled by assume-then-arbitrate, not a gate. For *provisioning* it's mostly benign — a stale decision provisions a now-redundant node (consolidation reclaims it) or hits a vanished offering (launch retries); both self-heal, unlike stale *binding*. The one sharp case — capacity appeared during the window, so the pod should now *bind* not provision — is already handled by D18: the pod is *nominated* to the NodeClaim, and when real capacity appears it re-enters scheduling, bind-first re-runs against fresh state, and it binds (the redundant claim is GC'd). No new phase; the requeue mechanism heals it. The real lever staleness imposes is the **Batch window length** (longer = denser packing but more staleness churn), not a Validate stage.

## Mapping to a known shape

This is MapReduce / a query planner, which is reassuring — it's a well-understood decomposition:
...
## Per-phase merits and demerits

### Batch() — load-bearing, well-motivated
**For:** Provisioning is intrinsically a set operation — the packing lookahead's headroom credit is *defined* over `remainingBatchDemand` and is unconstructable pod-at-a-time (per-pod provisioning produced the measured 375-node pathology). Batching is the floor, not an optimization. It also cleanly decouples provisioning latency from the per-pod bind hot path (binding stays synchronous; provisioning batches async). Karpenter already does this (batching window).
**Against / open:** The batch boundary is a real latency knob — bigger batch packs denser but delays provisioning, and is a larger staleness unit (more of the world changes underneath a long window). Not a flaw in the phase, but the window policy is a genuine tradeoff to own. Batch is arguably not a "phase" so much as the queue-drain boundary that turns a stream into the pipeline's input.

### Split() — the make-or-break phase
**For:** This is where both advantages get their teeth. Independent splits (a) solve in parallel — the goroutine win — and (b) keep each solve small, directly attacking the catalog-width/packing cost (O1). And splitting on genuine incompatibility is **exact, not an approximation**: a pod requiring `arm64` and one requiring `amd64` can *never* share a NodeClaim, so solving them together is pure wasted cross-product. Splitting them loses nothing and parallelizes for free.
**Against / the hazard:** **Compatibility is not transitive, so it is not a clean partition.** Pod A (`zone in {a,b}`) is compatible with B (`zone a`) and C (`zone b`), but B and C are not compatible with each other. There is no clean equivalence class. Options are all imperfect: over-split (isolate A → lose the packing it could have shared) or duplicate A across splits and reconcile at Select (complexity + double-provision risk). So Split must key on **hard incompatibility only** (arch, OS, zone-*if-pinned*, taints, DRA class) — the fault lines nothing can cross — and keep everything within a class together for packing.
**Two more constraints on the split key:**
...
- **Spread/anti-affinity are "keep-together" forces that fight splitting.** Pods that spread against each other, or repel each other, are *coupled* — Split must keep a coupled group in one split or it breaks the constraint. So topology makes splits coarser, reducing parallelism exactly where cross-pod constraints exist.
**Consequence:** Split's parallelism is **data-dependent**. A heterogeneous batch (many arches/zones/taints) shatters into many parallel splits; a homogeneous batch (all the same shape) is one split and gets zero parallelism. The phase is correct either way, but its headline benefit only materializes for diverse workloads.

### Solve() — a portfolio, justified only if the solvers are complementary
...
**Against / open:** Running *every* solver on *every* split is `N_solvers ×` the compute — only worth it if the solvers genuinely diverge. A portfolio of near-identical greedies pays N× for nothing. The phase should be explicitly framed as "a set of *complementary* heuristics," and Split() might also **route** (tiny zone-locked split → ILP; huge split → greedy) instead of blindly fanning out to all — routing trades some robustness for a lot of saved compute. Fan-out-all is the simple default; routing is the optimization.

### PostSolve() — optional; the home for anything that reads the candidate set
**What earned its existence:** the `(Problem, Solution[]) → Solution[]` signature is what makes warm-start expressible without a hack (a solver reading prior candidates as a bound) — and it turns out that same signature covers *everything* that consumes the candidate set: **ensemble-propose** (read the set, append a smarter candidate — this is the "solver that wants to see the others' options while making its own" case; it's an append, **not** a Select plugin, because Select chooses and never adds), **refine** (map — flexibility widening, spot retargeting, consolidation-merge à la PR #3008), and **prune** (filter). All three are the same shape; PostSolve is just "the stages downstream of the fan-out," unified.
**Why it's optional / v1 skips it:** with one trusted default solver there is nothing to warm-start from, no ensemble, and the solver already emits well-shaped NodeClaims — so PostSolve is empty and Select is identity. It exists in the *contract* (so warm-start/ensemble are additive later, no contract break) but ships as a no-op.
**Two costs the signature carries (log them):**
...
- **Feasibility-preservation is no longer guaranteed by the phase** — a `(Problem, Solution[])` stage can synthesize arbitrary candidates. `Solution.unplaced` + Select absorb this, but "does not strand a pod" demotes from a phase contract to an *optional declared property* of a given plugin.

### Select() — clean IF the upstream invariants hold
**For:** One decision point, one argmin, per split. Trivially parallel across splits. If splits are non-overlapping and PostSolve normalized the candidates, Select is a simple min over a common metric.
**Against / open:** Select inherits whatever Split and PostSolve didn't resolve. If Split allowed overlap (duplicated pods), Select must **reconcile/dedup** so a pod isn't provisioned twice — a global step that breaks the clean per-split parallelism. And the comparison metric is the same unsolved comparability question as the design doc's O2 (raw `$/hr` vs an abstract comparable; non-priced environments like fixed fleets have no per-offering price). If PostSolve normalizes, Select is trivial; if not, Select carries the whole comparability burden.

## Overall assessment

**The spine is sound and the two claimed advantages are real and important.** Ignoring existing nodes makes Solve a pure function — an architectural unlock neither Karpenter's `Solve` (coupled to cluster state) nor kube-scheduler (coupled to snapshot+cache) has. Parallelism via independent splits is the throughput unlock kube-scheduler's serialized cycle structurally cannot have. Both are *consequences of bind-first* — provisioning got simple enough to be pure and parallel only because binding was peeled off first.

**Confidence per phase:**
- **Batch, Solve, Select** — clearly load-bearing, well-motivated, keep.
- **Split** — powerful but conditional. Keep it, but define the key precisely (hard-incompatibility classes; never soft preferences; spread/anti-affinity force coarser splits). Its parallelism is data-dependent. This phase is where the design lives or dies.
- **PostSolve** — justified by the `(Problem, Solution[])` signature (warm-start / ensemble / refine / prune are all the same shape). Optional; ships as a no-op in v1. It is *the same interface as Solve* (empty-incumbent = solver), so it isn't really a separate phase to justify — it's the dependent-input wiring of the one `Refine` interface.
...
**Is the pipeline the right factoring?** Provisionally yes for Batch→Split→Solve→Select, with PostSolve as the optional dependent-stage wiring. **Plugin surfaces: Narrow (conjunctive, the constraints — the important one), Solve (competitive, the packing strategies), PostSolve (dependent). No Assign plugin (mechanism, not policy), no Cost plugin (offering data).** The joints to pressure-test before committing:
1. **Split correctness under non-transitive compatibility** — hard-incompat keys are exact; anything softer risks lost packing or double-provisioning. Get this wrong and the parallelism win comes at a packing-quality cost.
2. **Solver-decides-soft-Narrow** — the solver applies a soft Narrower by pricing the tighter-claim result against offerings; confirm this cleanly yields the D5 "soft prefs don't tip provisioning" property in the greedy loop.
...
There is no "Assign" plugin. An earlier draft invented one — a competitive packing-*algorithm* surface — but a packing algorithm is **mechanism, not policy**: no workload author ever asks for "best-fit-decreasing." It has no audience, so it is not a plugin surface. This maps exactly onto kube-scheduler: **the scheduling cycle is fixed; Filter and Score are the plugins.** Our analog:
...
**Eager is late with an early collapse**, so the superposition-of-superpositions structure *subsumes* eager rather than being forced into it. Late is the natural mode and the prize (no retry loop); eager falls out as a collapse policy. We are **not** locked to Karpenter's eager retry.
...
- **Eager vs late collapse of the source axis** — late (carry all ranks, collapse at commit, no retry) is the prize; confirm it holds under heterogeneous per-source requirements. Eager (collapse at construction) is the fallback and is a *special case* of the same structure, not a separate mechanism.
- **Neutral `CapacitySource` shape** — minimal `{offerings, requirements, rank}` that each provider's fallback concept compiles into; how per-source heterogeneous taints/labels/reqs intersect under one claim's nested superposition.
- **Rank vs Cost ordering** — O3: source rank is lexicographic, above the cost argmin; Cost never compares across sources. Confirm the collapse is cleanly two-phase (rank then cost).
- **Split key** — exact set of hard-incompatibility dimensions; how spread/anti-affinity coupling constrains it; whether to allow overlap + reconcile or forbid it.
- **Batch window** — time vs count vs queue-drain; the latency/packing/staleness tradeoff.
- **Solve routing vs fan-out-all** — always run every solver, or route splits to suitable solvers?
- **Select metric** — reads effective offering price off the solution (cost is offering data, not a plugin); falls back to node-count/utilization for non-priced fleets. Confirm this is the whole comparator.
- **Cross-split packing loss** — do we ever want a global re-pack across splits, or is intra-split packing enough?
- **Is Batch a phase or just the input boundary?**
- **Soft-preference home** — resolved: soft prefs are soft Narrowers the solver optionally applies (no Score surface); performance-value ("new gen 20% better") is offering data (no `PreferenceDiscount` knob). Remaining: exact solver policy for *when* to honor a soft Narrower (price threshold vs. flexibility tradeoff).

## Wiring is the deliverable (not the plugins)

What actually gets written into kube-scheduler is **(1) the wiring and (2) a default solver** — not a rich plugin set. The plugin interface (`Refine`, `Select`) is table stakes; the wiring is the opinionated, permanent, hard-to-reverse part every plugin's contract depends on, so that's where the design care goes.

- **Commit the fan-out *shape* now, ship one solver.** The contract is always "Solve produces a set → Select reduces it"; v1 registers **one** solver, so the set is a singleton and Select is identity — a plain call, no goroutines. Registering a 2nd solver later turns on fan-out with **zero contract change**. Fan-out is unretrofittable cheaply (a plugin can't grant itself a portfolio — only the orchestrator can invoke N solvers), so the *shape* must be in from day one; the *parallelism* and *portfolio* are additive behind it.
...
- **Pin one topology, don't build a DAG engine.** `(Problem, Solution[]) → Solution[]` *permits* arbitrary dataflow; the wiring must fix exactly one shape — parallel fan-out → (optional ordered PostSolve) → Select — not a general plugin-graph interpreter.
- **The default solver already exists:** the current inline `solve()` (greedy largest-first + packing lookahead + `NarrowForPod` + topology injection, 33 tests green) repackaged behind `Refine(Problem, ∅)`. The default is repackaging, not new work.
